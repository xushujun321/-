        ...
        <div v-if="type === 'personal'">
          ...
          <table cellspacing="0" width="100%" class="tableList">
            ...
            <tr>
              <th>员工照片</th>
              <td>
                <img :src="formData.staffPhoto" alt="">
              </td>
              <th>生日</th>
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      ...
      const userInfo2 = await getPersonalDetail(this.userId) // 获取个人基本信息
      this.formData = { ...userInfo2, ...userInfo }
      // $ vue的方法
      // 同步 和 异步 先执行同步
      // 页面渲染是异步 执行数据更新之后，如果直接去打印，页面的数据肯定没准备好
      // 需要等待上一次的数据更新 渲染完成  才去打印数据
      // this.$nextTick表示当里面的回调函数执行完成的时候 上一次的数据更新已经异步渲染完成了
      this.$nextTick(() => {
        window.print()
      })
    //   this.$nextTick()
